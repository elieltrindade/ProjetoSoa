...
public class BuscadorUF {
	...
	@WebMethod(action = "buscarUFsPorCapital")
	public ListaUF buscarUFsPorCapital(@WebParam(name = "busca")String busca) throws Exception{
		ListaUF ufs = new ListaUF();
		for (String uf[] : listaUFs) {
	...
	@WebMethod(action = "obterUfsComPopulaçãoMaiorQue")
	public ListaUF obterUfsComPopulaçãoMaiorQue(@WebParam(name = "busca")int busca) throws Exception{
		ListaUF ufs = new ListaUF();
		for (String uf[] : listaUFs) {
			if (Integer.parseInt(uf[2]) > busca) {
				ufs.getUFs().add(new UF(uf[0], uf[1], uf[2]));
			}
		}
		return ufs;
	...
	@WebMethod(action = "obterUfsComPopulaçãoMenorQue")
	public ListaUF obterUfsComPopulaçãoMenorQue(@WebParam(name = "busca")int busca) throws Exception{
		ListaUF ufs = new ListaUF();
		for (String uf[] : listaUFs) {
			if (Integer.parseInt(uf[2]) < busca) {
				ufs.getUFs().add(new UF(uf[0], uf[1], uf[2]));
